  ...
  return (
    <main className="min-h-screen bg-brand-bg print:bg-white">
      <div className="max-w-xl mx-auto px-5 py-8">
        ...
        {/* ── Report sections — only rendered when report exists ────────────── */}
        {!reportFailed && report && (() => {
          ...
          return (
            <>
              ...
              <SectionCard title="5. Cost Preparation Guide">
                <p className="text-sm text-brand-muted mb-3">Key factors that affect cost for this type of work:</p>
              ...
              <SectionCard title="6. Questions to Ask Every Contractor">
                <ol className="space-y-5">
                  {report.questionsToAsk.map((q, i) => (
                    <li key={i} className={i < report.questionsToAsk.length - 1 ? 'pb-5 border-b border-brand-border' : ''}>
                      <p className="text-sm font-semibold text-brand-navy mb-1.5">{i + 1}. {q.question}</p>
                ...
                <div className="card">
                  ...
                  <ul className="space-y-2">
                    {report.redFlags.map((flag, i) => (
                      <li key={i} className="text-xs text-red-700 bg-red-50 rounded-lg px-3 py-2 leading-relaxed">{flag}</li>
                    ))}
                  </ul>
                </div>
                <div className="card">
                  ...
                  <ul className="space-y-2">
                    {report.insistOnWriting.map((item, i) => (
                      <li key={i} className="text-xs text-emerald-800 bg-emerald-50 rounded-lg px-3 py-2 leading-relaxed">{item}</li>
                    ))}
                  </ul>
                </div>
              </div>
        ...
        {/* ── Chat — always available after payment ─────────────────────────── */}
